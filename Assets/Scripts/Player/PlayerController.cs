...
public class PlayerController : MonoBehaviour
{
    ...
    public bool PlayerSprints 
    { get => playerSprints; set => playerSprints = value; } 
    [SerializeField]
    ...
    public float MaxEndurance { get => maxEndurance; set => maxEndurance = value; }
    ...
    void Start()
    {
        ...
        // Lade Sensitivity Einstellung
        Sensitivity = Preferences.instance.Load_Sensitivity();
    ...
    /// <summary>
    /// DeadScene laden, wenn der Spieler tot ist
    /// </summary>
    ...
    /// <summary>
    /// Setzt den Spieler zu Startposition zurueck wenn er aus denm Spiel faellt
    /// </summary>
    ...
    void PlayerMovement()
    {
        ...
        {      
            ...
            if (!enduranceIsRecovery && Endurance < maxEndurance 
                && !playerSprints)
            {
                StartCoroutine(ResetSprintEndurance(waitTimeForEnduranceReset, 
                    maxEndurance));
            }
            ...
            if (rotatePlayerWithButtons)
            {
                Quaternion deltaRotation = Quaternion.Euler
                    (0, keyInput.x * rotationSpeed * Time.deltaTime, 0);
                playerBody.MoveRotation(playerBody.rotation * deltaRotation);
            }
            else
            {
                // Zur Seite gehen
                playerBody.MovePosition(playerBody.position + 
                    speed * Time.deltaTime * keyInput.x * transform.right);
            }

            // Nach vorne gehen
            playerBody.MovePosition(playerBody.position + 
                speed * Time.deltaTime * keyInput.z * transform.forward);
        }
    }

    /// <summary>
    /// Erhöht die Geschwindigkeit des Player 
    /// Unterberücksichtigung der Ausdauer
    /// </summary>
    ...
    {     
        if (Endurance > 0)
        {
            ...
            {
                StopCoroutine(ResetSprintEndurance(waitTimeForEnduranceReset, 
                    maxEndurance));
    ...
    /// <returns></returns>
    IEnumerator ResetSprintEndurance(float _waitTimeForReset, 
        float _enduranceMaxLimit)
    {
    ...
    void PlayerRotating()
    {
        ...
        cameraAngle += -mouseInput.y * rotationSpeed * 
            sensitivity * Time.deltaTime;

        // Winkel der Kamera auf min und max begrenzen
        cameraAngle = Mathf.Clamp(cameraAngle, -maxVerticalCameraAngle, 
            maxVerticalCameraAngle);
        // Debug.Log(cameraAngle);

        // Rotationswinkel zuweisen
        camTransform.transform.localEulerAngles = 
            new Vector3(cameraAngle, 0, 0);

        // Spieler dreht sich nach links oder rechts
        Quaternion deltaRotationX = Quaternion.Euler(0, rotationSpeed * 
            sensitivity * Time.deltaTime * mouseInput.x, 0);
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        else if (collision.gameObject.CompareTag("Enemy"))   //tag = "Enemy"
        {
            phealth -= edamage;
        }
    ...
    private void OnCollisionStay(Collision collision)
    {
        if (collision.gameObject.CompareTag("Wall") || 
            collision.gameObject.CompareTag("ExitGate"))
        {
            hitWall = true;
        }
        else
