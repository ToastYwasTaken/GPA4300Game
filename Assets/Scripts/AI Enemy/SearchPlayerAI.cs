...
public class SearchPlayerAI : MonoBehaviour
{
    ...
    /// <summary>
    /// Löst die Suche nach dem Spieler aus
    /// </summary>
    public void StartSearchingPlayer()
    {
        if (player)
        {
            ...
            InvokeRepeating(nameof(SearchingPlayer), searchingTime, 
                repeatRate);
        }
        else
        {
        ...
        if (Physics.Raycast(ray, out RaycastHit raycast, viewMaxDistance))
        {
            if (raycast.collider.gameObject == player)
            {
                Vector3 rayDirection = 
                    raycast.transform.position - transform.position;
    ...
    /// <summary>
    /// Wartet eine bestimmte Zeit und setzt danach isPlayerDetected auf false
    /// </summary>
    /// <returns></returns>
